<?php
	if(isset($_GET['ex'])){
		...
		try{
			$conexao->query("DELETE FROM tb_estoque WHERE id = $ex");
		}catch(PDOException $ex){
			echo "<script>alert('Existem registros relacionados a este!')</script>";
		}
		
		echo "<meta http-equiv='refresh' content='0, url=?p=".base64_encode('estoques')."'>";
	}
?>
								...
								<form action="" method="post">
									<div class="modal-content">
										...
										<div class="modal-footer">
											...
											<a href = "<?php echo '?p='.base64_encode('estoques').'&ex='.base64_encode($RSBusca['id']);?>" class="btn btn-primary">Confirmar</a>
										</div>
								</form>
								</div>
							</div>
                       ...
                       <div class="row">
                            <div class="col-md-12">
                                <div class="overview-wrap">
                                    <h2 class="title-1">Estoques Cadastrados</h2>
                                    
										<a href = "?p=<?php echo base64_encode('novoestoque');?>" class="au-btn au-btn-icon au-btn--blue">
											<i class="zmdi zmdi-plus"></i>add Estoque
										</a>
											...
											<?php
												...
												while($RSBusca = $SQLBusca->fetch()){
													...
													echo "<tr>
														  ...
														  <td width = '10%'><a href='?p=".base64_encode('editaestoque')."&id=".base64_encode($RSBusca['id'])."' title = 'Editar Setor'><i class = 'fa fa-edit'></i></a></td>
														  <td>".$RSBusca['nome']."</td>";
