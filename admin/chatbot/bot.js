    ...
    connection.query("SELECT * FROM tb_pessoa WHERE fone LIKE '"+num+"'", function(err, rows){
        ...
        if(!err){
            ...
            if(rows.length > 0){
                ...
                connection.query("SELECT * FROM tb_botchamado WHERE pessoa = "+pessoa+" ORDER BY id DESC LIMIT 1", function(err, rowsChamado){
                            ...
                            if(parametro === "INICIA_CONVERSA"){
                                    ...
                                    if(!err){
                                        if(rowsCat.length > 0){
                                            connection.query("INSERT INTO tb_botchamado(pessoa,parametro,valor) VALUES('"+pessoa+"','CATEGORIA','"+categoria+"')", function(err){
                                                if(!err){
                                                    client.sendMessage(message.from, 'Agora, em uma mensagem escreva sua solicitação para finalizarmos a abertura desse chamado');
                                ...
                                connection.query("INSERT INTO tb_botchamado(pessoa,parametro,valor) VALUES('"+pessoa+"','SOLICITACAO','"+solicitacao+"')", function(err){
                                    if(!err){
                                        ...
                                        //recupera dados
                                        console.log("SELECT * FROM tb_botchamado WHERE pessoa = '"+pessoa+"'");
                                        ...
                                        connection.query("SELECT * FROM tb_botchamado WHERE pessoa = '"+pessoa+"'", function(err, rowsDados){
                                            if(!err){
                                                ...
                                                console.log("INSERT INTO tb_chamado(data,hora,descricao,tb_categoria_id,tb_departamento_id,tb_pessoa_id,protocolo) VALUES('"+data+"','"+horaSimples+"','"+recSolicitacao+"','"+recCategoria+"','"+recDepartamento+"','"+pessoa+"','"+protocolo+"')");
                        ...
                        if(rows.length > 0){
                            ...
                            connection.query("INSERT INTO tb_botcadastro(fone,parametro,valor) VALUES('"+num+"','INICIA_CADASTRO','1')", function(err){
                                if(!err){
                                    ...
                                    client.sendMessage(message.from, 'Este é um canal exclusivo para abertura de chamados da secretaria de saúde.');
